...
(async function () {
  ...
  logger(`${api.getCurrentUserID()} - [ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "This bot was made by SHAON AHMED" : global.config.BOTNAME}`, "[ BOT INFO ]");
  ...
  return (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`😘আমার বস RAJA এর কাছে রিকোয়েস্ট পাঠিয়ে দেও হয়েছে....🍂🌸`, event.threadID);
      }
...
🖤𝚈𝚘𝚞 𝙲𝚊𝚗 𝙲𝚊𝚕𝚕 𝙷𝚒𝚖 〠RAJA 01715559179\n

😳𝐇𝐢𝐬 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝🤓:-https://www.facebook.com/profile.php?id=100088847116551 \n

📑 গ্রুপে পারমিশন না পেলে আমার বস কে এড করুন💖\n
Contact On মেসেঞ্জার  Username:m.me/RAJA.ViP.5X.09638357510`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
